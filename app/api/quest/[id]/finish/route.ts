import { finishQuestUseCase } from "@/application/usecase/finishQuestUseCase";
...
export async function POST(req: Request, { params }: {params: { id: number } }) {
    try {
            ...
            await finishQuestUseCase(questRepo, id, profile.id);
    
            return NextResponse.json({ message: "Quest successfully updated to finish" }, { status: 200 });
        } catch (error) {
