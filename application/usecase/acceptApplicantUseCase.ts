...
export async function acceptApplicantUseCase(questRepo: questRepository, questId: number, posterId: number, freelancerId: number) {
    ...
    if (existingQuest.poster_id !== posterId) throw new Error("Only quest poster can accept applicant");
    if (existingQuest.poster_id === freelancerId) throw new Error("Quest poster cannot accept themself");
    ...
    if (existingQuest.freelancer_id) throw new Error("Freelancer already exist");
